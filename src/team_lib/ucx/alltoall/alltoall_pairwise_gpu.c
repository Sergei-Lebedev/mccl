...
xccl_status_t xccl_ucx_alltoall_pairwise_gpu_progress(xccl_ucx_collreq_t *req)
{
    ...
    int            total_reqs = (chunk > group_size || chunk <= 0) ?
        group_size : chunk;
    void *scratch = req->alltoall_pairwise.scratch;
    ...
    while (n_polls++ < max_polls &&
        ...
        if (req->alltoall_pairwise.n_rreqs < group_size) {
            ...
            if (XCCL_OK == status) {
                ...
                xccl_ucx_recv_nb((void*)((ptrdiff_t)scratch + peer*data_size + group_size*data_size),
        ...
        if (req->alltoall_pairwise.n_sreqs < group_size) {
            ...
            if (XCCL_OK == status) {
                ...
                xccl_ucx_send_nb((void*)((ptrdiff_t)scratch + peer*data_size),
    ...
    cudaMemcpy(req->args.buffer_info.dst_buffer,
               (void*)((ptrdiff_t)scratch + group_size*data_size),
               group_size*data_size,
               cudaMemcpyDeviceToDevice);
    xccl_mem_component_free(scratch, req->mem_type);
    free(reqs);
...
xccl_status_t xccl_ucx_alltoall_pairwise_gpu_start(xccl_ucx_collreq_t *req)
{
    ...
    xccl_mem_component_alloc(&scratch,
                             2*group_size*data_size, UCS_MEMORY_TYPE_CUDA);
    cudaMemcpy(scratch, req->args.buffer_info.src_buffer,
               group_size*data_size,
               cudaMemcpyDeviceToDevice);
    ...
    req->progress = xccl_ucx_alltoall_pairwise_gpu_progress;
    ...
    req->alltoall_pairwise.scratch = scratch;
